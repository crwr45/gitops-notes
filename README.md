...
Some fairly crude Ansible playbooks are included that automate the configuration of two clusters with OpenShift GitOps. `site.yml` will install GitOps on the source cluster, grant it cluster admin permissions. Then on the target cluster create a Namespace, ServiceAccount, give it permissions, and then add a record back into the source cluster to allow ArgoCD to authenticate against the target cluster and manage it.
